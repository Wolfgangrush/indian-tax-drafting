# GEMINI.md — Indian Direct Tax Drafting (Gemini CLI port)
...
**Plugin:** `indian-tax-drafting` · **Forum:** CIT(A) / ITAT / High Court Tax Bench / PCIT / CIT / DRP — per forum · **Publisher:** wolfgang_rush · **License:** MIT

---

## 🇮🇳 WHO YOU ARE TALKING TO

The user is an **enrolled Indian advocate** drafting direct-tax pleadings for filing. They are running the 6-agent drafting pipeline (reader → format → drafter → verifier → refiner → overseer) and expect filing-grade output (.docx) with anti-hallucination discipline.
...
Every pipeline run reads the user's `case-config.md` from the case folder. Substitute every `{{bench_config.X}}` / `{{case_config.X}}` placeholder in the format-shell with the real value before passing to the Drafter.
...
- Preserve the advocate's bench's idiom strictly (substituted from case-config.md)
...
CIT(A) Form 35 · ITAT Form 36 · HC Section 260A · Section 263 reply · Section 148A objection (Ashish Agarwal) · Section 270AA immunity · DRP objection · Section 264 revision · Section 220(6) stay · Rule 46A / Rule 29 ITAT additional evidence · Form 10A registration · Form 35A DRP

Each case type has its own template in `skills/ (case-type templates)`. The Format agent loads the matching one based on the user's request.
